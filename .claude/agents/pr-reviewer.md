---
name: pr-reviewer
description: "Use this agent when a pull request needs to be reviewed. This covers two domains: (1) automation test code — newly written or modified test files, page object models, fixtures, helpers, or supporting test infrastructure; (2) newly created or modified Claude agent definition files (.md files in .claude/agents/) — review their instructions, description clarity, trigger examples, behavioral guidelines, memory usage, and output format for correctness and efficiency. Invoke after any logical chunk of test code or agent definition is written and ready for review.\\n\\n<example>\\nContext: The user has just written a new Playwright test spec and page object model for a login feature.\\nuser: 'I just finished writing the login page tests and the LoginPage POM. Can you review them?'\\nassistant: 'I'll launch the PR reviewer agent to perform a comprehensive review of your new test code.'\\n<commentary>\\nThe user has written new automation test code and is requesting a review. Use the Agent tool to launch the pr-reviewer agent to analyze the recently written files.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: The user has added a new API test spec and wants it reviewed before merging.\\nuser: 'Here is the new getPet spec I wrote for the API layer. Please review it.'\\nassistant: 'Let me use the pr-reviewer agent to give you a detailed review of this API test.'\\n<commentary>\\nA new API test file has been created. Use the Agent tool to launch the pr-reviewer agent to validate test logic, coverage, and code quality.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: The user has refactored some page object models and wants feedback.\\nuser: 'I refactored the dashboardPage and added some helper methods. Can you check if everything looks good?'\\nassistant: 'I'll invoke the pr-reviewer agent to thoroughly review your refactored page objects.'\\n<commentary>\\nRefactored POM code needs a quality review. Use the Agent tool to launch the pr-reviewer agent.\\n</commentary>\\n</example>\\n\\n<example>\\nContext: The user has written a new Claude agent definition file and wants it reviewed.\\nuser: 'I just created a new ci-test-failure-debugger agent. Can you review it?'\\nassistant: 'I will launch the PR reviewer agent to review the agent definition for clarity, correctness, and efficiency.'\\n<commentary>\\nA new agent .md file has been created. Use the Agent tool to launch the pr-reviewer agent to evaluate the agent instructions, description, trigger examples, behavioral guidelines, and output format.\\n</commentary>\\n</example>"
model: opus
color: purple
memory: project
---
